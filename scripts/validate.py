...
验证用例是否符合 XMind 导入格式规范：
- 用例声明：#### tc-P1-功能测试: 标题
- 前置条件：* pc: 内容
- 操作步骤：* 步骤描述
- 预期结果：    * 预期结果（4空格缩进）
- 测试类型：只能是「功能测试」或「性能测试」

功能：
1. 格式验证：检查用例是否符合 XMind 导入格式
...
def is_state_contrast_pair(detail: Dict[str, float]) -> bool:
    """识别状态对比型并行用例：步骤几乎一致但预期完全不同。

    这是状态机/权限矩阵维度刻意设计的对比组（如"已创建删除按钮置灰" vs "进行中删除按钮置灰"），
    步骤相同但断言完全不同，是覆盖意图而非重复。
    """
    return detail['steps'] >= 0.95 and detail['expected'] <= 0.30
...
def detect_duplicates(cases_with_path: List[Tuple[TestCase, str]],
    ...
    for i in range(len(cases_with_path)):
        for j in range(i + 1, len(cases_with_path)):
            ...
            if similarity >= threshold and not is_state_contrast_pair(detail):
